...
<script>
    $(document).ready(function() {
        ...
        $(document).on('click', '.editBtn', function () {
            var id = $(this).data('id');
            var url = "{{ route('employee.wash.edit', ":id") }}";
        ...
        $('#editForm').submit(function (event) {
            event.preventDefault();
            var id = $('#wash_id').val();
            var url = "{{ route('employee.wash.update', ":id") }}";
            url = url.replace(':id', id)
            $.ajax({
                url: url,
                type: "PUT",
        ...
        $(document).on('click', '.deleteBtn', function(){
            var id = $(this).data('id');
            var url = "{{ route('employee.wash.destroy', ":id") }}";
            url = url.replace(':id', id)
            Swal.fire({
                title: 'Are you sure?',
        ...
        $('#trashedDataTable').DataTable({
            ...
            searching: true,
            ajax: {
                url: "{{ route('employee.wash.trashed') }}",
            },
            columns: [
                { data: 'wash_name', name: 'wash_name' },
                { data: 'action', name: 'action', orderable: false, searchable: false }
        ...
        $(document).on('click', '.restoreBtn', function () {
            var id = $(this).data('id');
            var url = "{{ route('employee.wash.restore', ":id") }}";
            url = url.replace(':id', id)
            $.ajax({
                url: url,
                type: "GET",
        ...
        $(document).on('click', '.forceDeleteBtn', function(){
            var id = $(this).data('id');
            var url = "{{ route('employee.wash.force.delete', ":id") }}";
            url = url.replace(':id', id)
            Swal.fire({
                title: 'Are you sure?',
        ...
        $(document).on('click', '.statusBtn', function () {
            var id = $(this).data('id');
            var url = "{{ route('employee.wash.status', ":id") }}";
            url = url.replace(':id', id)
            $.ajax({
                url: url,
                type: "GET",
